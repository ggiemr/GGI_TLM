...
!
! SUBROUTINE read_output_volume_average_list
!
! NAME
!     read_output_volume_average_list
!
! DESCRIPTION
...
! START  
  ...
  if ( allocated( output_volume_averages ) ) GOTO 9000
  
  ALLOCATE ( output_volume_averages(1:n_output_volume_averages) )

  do output_number=1,n_output_volume_averages
    ...
    CALL read_output_time_information(input_file_unit,	&
                                      output_volume_averages(output_number)%specified_timestep_information,	&
                                      output_volume_averages(output_number)%first_timestep,	&
                                      output_volume_averages(output_number)%last_timestep,	&
                                      output_volume_averages(output_number)%timestep_interval,	&
                                      output_volume_averages(output_number)%specified_time_information,	&
                                      output_volume_averages(output_number)%first_time,	&
                                      output_volume_averages(output_number)%last_time,	&			      
                                      output_volume_averages(output_number)%time_interval )
    
  end do ! next output volume_average

  CALL write_line('FINISHED: read_output_volume_average_list',0,output_to_screen_flag)
